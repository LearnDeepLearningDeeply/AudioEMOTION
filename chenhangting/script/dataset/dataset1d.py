...
class AudioFeatureDataset(Dataset):
    def __init__(self,featrootdir,cvtxtrootdir,feattype,cvnum,train,normflag,normfile):
        self.__pathfeatrootdir=featrootdir
        if(train):
            self.__pathcvtxt=os.path.join(cvtxtrootdir,'fold{}_train.txt'.format(cvnum))
        else:
            self.__pathcvtxt=os.path.join(cvtxtrootdir,'fold{}_test.txt'.format(cvnum))
        ...
        if(self.normflag==1 and self.train==False):warnings.warn("You want to calculate normalization coefficients according to test dataset")
