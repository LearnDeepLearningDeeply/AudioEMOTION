...
class Net(nn.Module):
    ...
    def fix(self):
        ...
        self.add_module('layerOut',nn.Sequential(
            nn.Linear(self.hidden_dim*self.r+2*self.input_dim,self.output_dim),
            nn.LogSoftmax(dim=1),
        ))

        self.add_module('attention1',nn.Sequential(
            nn.Linear(self.hidden_dim,self.da,bias=True),
            nn.BatchNorm1d(self.da),
            nn.Tanh(),
    ...
    def forward(self,x,length):
        ...
        for idx,l in enumerate(length):
            ...
            penalty_mat[idx,:,:]=torch.pow(torch.matmul(torch.sqrt(A_idx_softmax),torch.transpose(torch.sqrt(A_idx_softmax),0,1))-one,2.0)
