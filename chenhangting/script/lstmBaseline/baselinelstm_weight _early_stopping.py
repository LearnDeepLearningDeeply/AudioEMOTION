...
def train_batch(ingredientWeight,data,target,length,name):
        batch_size=data.size(0)
...
#            print(torch.squeeze(output[i,0:length[i],:]));print(torch.squeeze(target[i,0:length[i]]))
        ...
        loss.backward()
        optimizer.step()
        return batch_size,loss.data[0]

def test_batch(data,target,length,name):
        test_lossBatch=0;numframesBatch=0
        test_dict1Batch={};test_dict2Batch={}
        batch_size=data.size(0)
        max_length=torch.max(length)
        data=data[:,0:max_length,:];target=target[:,0:max_length]
        ...
        for i in range(batch_size):
            ...
            test_dict1Batch[name[i]]=result
            test_dict2Batch[name[i]]=target.cpu().data[i][0]
            test_lossBatch+=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]),size_average=False).data[0]
            numframesBatch+=length[i]
        return numframesBatch,test_lossBatch,test_dict1Batch,test_dict2Batch

def train(epoch,trainLoader):
    model.train()
    for batch_inx,(data,target,length,name) in enumerate(trainLoader):
        batch_size,loss_num=train_batch(trainLoader.dataset.ingredientWeight,data,target,length,name)
        if(batch_inx % args.log_interval==0):
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_inx * batch_size, len(trainLoader.dataset),
                100. * batch_inx / len(trainLoader), loss_num))
...
def test(testLoader):
    ...
    for data,target,length,name in testLoader:
        numframesBatch,test_lossBatch,test_dict1Batch,test_dict2Batch=test_batch(data,target,length,name)
        numframes+=numframesBatch
        test_loss+=test_lossBatch
        test_dict1={**test_dict1,**test_dict1Batch}
        test_dict2={**test_dict2,**test_dict2Batch}
        #need something
