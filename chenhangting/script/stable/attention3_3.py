...
normfile=r'./temp3/ms{}.npy'.format(args.cvnum)
...
def train(epoch,trainLoader):
    ...
    for batch_inx,(data,target,length,name) in enumerate(trainLoader):
        ...
        optimizer.zero_grad()
        output,_,penalty=model(data,length,superParams['r'])
        numframes=0 
...
#        weight_loss/=float(param_num);grad_total/=float(param_num)
        ...
        optimizer.step()
        print('Train Epoch: {} [{}/{} ({:.0f}%)]\tAve loss: {:.6f} and Total weight loss {:.6f} and penalty {:.6f} and Total grad fro-norm {:.6f}'.format(
...
def test(testLoader):
    ...
    for data,target,length,name in testLoader:
        ...
        with torch.no_grad():output,_,penalty=model(data,length,superParams['r'])
