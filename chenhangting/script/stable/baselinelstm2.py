...
def train(epoch,trainLoader):
    ...
    for batch_inx,(data,target,length,name) in enumerate(trainLoader):
        ...
        for i in range(batch_size):
            ...
            if(i==0):loss=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]),size_average=True)*emotionLabelWeight[label]
            else:loss+=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]),size_average=True)*emotionLabelWeight[label]
            numframes+=length[i]
        loss.backward()
...
#        weight_loss/=float(param_num);grad_total/=float(param_num)
        ...
        print('Train Epoch: {} [{}/{} ({:.0f}%)]\tAve loss: {:.6f} and Total weight loss {:.6f} and Total grad fro-norm {:.6f}'.format(
            ...
            100. * batch_inx / len(trainLoader), loss.item()/len(trainLoader.dataset),weight_loss,grad_total))
...
def test(testLoader):
    ...
    for data,target,length,name in testLoader:
        ...
        for i in range(batch_size):
            ...
            test_loss+=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]),size_average=True).item()*emotionLabelWeight[label]
            numframes+=length[i]
...
#        print(np.argmax(result)==test_dict2[filename])
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
        test_loss/len(test_dict1), metrics.accuracy_score(label_true,label_pred,normalize=False), \
